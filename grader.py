...
Returns scores strictly in (0.05, 0.95) — never 0.0 or 1.0.

Formula: score = 0.2 + (fixed_issues / total_issues) * 0.6 + 0.2 * validation_pass
Clamped:  score = min(0.95, max(0.05, score))
"""
...
def grade(task_id: str, episode_result: Dict[str, Any]) -> float:
    """
    Return score strictly in (0.05, 0.95).

    score = 0.2 + (fixed_issues / total_issues) * 0.6 + 0.2 * validation_pass
    Clamped: min(0.95, max(0.05, score))
...
if __name__ == "__main__":
    tests = [
        ...
        ("task_eval_auth_flaw",          0, False, 0.20),   # nothing fixed
        ("task_eval_auth_flaw",          2, False, 0.50),   # half fixed
        ("task_eval_auth_flaw",          4, True,  0.95),   # all fixed + victory → clamped
        ("task_insecure_deserialization",0, False, 0.20),
        ("task_insecure_deserialization",1, False, 0.50),
        ("task_insecure_deserialization",2, True,  0.95),   # clamped
        ("task_command_injection",       0, False, 0.20),
        ("task_command_injection",       3, True,  0.95),   # clamped
        ("task_hardcoded_key",           1, True,  0.95),   # clamped
        ("task_vulnerable_dep",          0, False, 0.20),
        ("task_vulnerable_dep",          3, True,  0.95),   # clamped
    ]
