...
Returns scores strictly in (0.06, 0.94) — never 0.0, 0.05, 0.95, or 1.0.

Formula: score = 0.2 + (fixed_issues / total_issues) * 0.6 + 0.2 * validation_pass
Clamped:  score = min(0.94, max(0.06, score))
"""
...
def grade(task_id: str, episode_result: Dict[str, Any]) -> float:
    """
    Return score strictly in (0.06, 0.94).

    score = 0.2 + (fixed_issues / total_issues) * 0.6 + 0.2 * validation_pass
    Clamped: min(0.94, max(0.06, score))

    episode_result keys:
      fixed_issues   (int)  — number of issues resolved this episode
      victory        (bool) — True if agent completed all objectives
      total_reward   (float)— fallback if fixed_issues not present
    """
    ...
    validation_pass = int(victory)

    # ── Division-by-zero guard ────────────────────────────────────────────────
    if total_issues == 0:
        ratio = 0.0
    else:
        ratio = min(fixed_issues, total_issues) / total_issues

    raw   = 0.2 + ratio * 0.6 + 0.2 * validation_pass
...
if __name__ == "__main__":
    tests = [
        ...
        ("task_eval_auth_flaw",          4, True,  0.94),   # all fixed + victory → clamped to 0.94
        ("task_insecure_deserialization",0, False, 0.20),
        ("task_insecure_deserialization",1, False, 0.50),
        ("task_insecure_deserialization",2, True,  0.94),   # clamped to 0.94
        ("task_command_injection",       0, False, 0.20),
        ("task_command_injection",       3, True,  0.94),   # clamped to 0.94
        ("task_hardcoded_key",           1, True,  0.94),   # clamped to 0.94
        ("task_vulnerable_dep",          0, False, 0.20),
        ("task_vulnerable_dep",          3, True,  0.94),   # clamped to 0.94
    ]
