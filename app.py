...
@app.get("/state")
async def state():
    """Return current env state including score strictly in (0.01, 0.99)."""
    global _env, _obs
    if _obs is None:
        return JSONResponse({"error": "call /reset first"}, status_code=400)
    score = _env._compute_score(_env._episode_reward)
    return JSONResponse({
        "observation":    _obs.model_dump(),
        "score":          score,
        "episode_reward": _env._episode_reward,
        "victory":        _env._victory,
