...
def grade_task(task_id: str, episode_result: Dict[str, Any]) -> float:
    ...
    Returns a score strictly in (0.05, 0.95) — never 0.0 or 1.0.
    """
    fn = _TASK_GRADERS.get(task_id)
    if fn is None:
        # Fallback: normalise episode reward
        reward  = float(episode_result.get("total_reward", 0.0))
        victory = bool(episode_result.get("victory", False))
        base    = 0.2 + 0.2 * int(victory)
        return round(min(0.95, max(0.05, base + reward * 0.01)), 4)
...
def _grade_task_hardcoded_key(episode_result: Dict[str, Any]) -> float:
    """Grader for task_hardcoded_key. 1 issue total."""
    total_issues    = 1
    fixed_issues    = int(episode_result.get("fixed_issues", 0))
    validation_pass = int(bool(episode_result.get("victory", False)))
    raw = 0.2 + (min(fixed_issues, total_issues) / total_issues) * 0.6 + 0.2 * validation_pass
...
def _grade_task_vulnerable_dep(episode_result: Dict[str, Any]) -> float:
    """Grader for task_vulnerable_dep. 3 issues total."""
    total_issues    = 3
    fixed_issues    = int(episode_result.get("fixed_issues", 0))
    validation_pass = int(bool(episode_result.get("victory", False)))
    raw = 0.2 + (min(fixed_issues, total_issues) / total_issues) * 0.6 + 0.2 * validation_pass
...
# Wire graders after all functions are defined
_TASK_GRADERS.update({
    "task_hardcoded_key":            _grade_task_hardcoded_key,
    "task_vulnerable_dep":           _grade_task_vulnerable_dep,
