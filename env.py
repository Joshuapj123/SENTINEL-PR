...
NO_VICTORY_REWARD_CAP = 1.0   # episode reward capped if no victory
...
class Action(BaseModel):
    ...
    resolved_issues:  Dict[str, List[str]] = Field(default_factory=dict)
    remaining_issues: Dict[str, List[str]] = Field(default_factory=dict)
    ...
    def _hard_validate(self) -> "Action":
        ...
        if at is ActionType.FLAG_VULN:
            ...
            if not self.patched_source or not self.patched_source.strip():
                raise ValueError("PROPOSE_PATCH: `patched_source` is required and must be non-empty.")
            if not self.fix_rationale or not self.fix_rationale.strip():
                raise ValueError("PROPOSE_PATCH: `fix_rationale` is required (explain WHY this fix is correct).")
            if not self.evidence:
                raise ValueError("PROPOSE_PATCH: `evidence` is required (cite the original vulnerability).")
...
class PatchValidator:
    ...
    def _sandbox(cls, source: str) -> None:
        ...
        if forbidden.search(source):
            return   # dangerous calls – skip exec, bandit already catches
        ...
        ).split() if hasattr(_b, k)}
        safe["__import__"] = __import__
        restricted = {"__builtins__": safe}
...
class SentinelPREnv:
    ...
    def step(self, action_dict: Any) -> StepResult:  # noqa: C901
        ...
        if isinstance(action_dict, Action):
            action = action_dict   # already validated
        else:
            try:
        ...
        if at is ActionType.FLAG_VULN:
            ...
            if not validation.passed:
                ...
                new_source    = action.patched_source
                new_manifest  = action.patched_manifest or self._current_obs.dependency_manifest
                patch_diff    = _unified_diff(self._current_obs.source_code, new_source)
                self._patch_applied = True

                after_issues  = _extract_issues(new_source, new_manifest)
                after_keys    = {i.key() for i in after_issues}
            ...
            if current_issues:
                reward += 1.0   # correct reject
                breakdown["correct_reject"] = 1.0
            else:
