...
class SentinelPREnv:
    ...
    SCORE_RANGES = {
        ...
        "task_insecure_deserialization": {"min": -3.0,  "max": 6.0},
        "task_command_injection":        {"min": -3.0,  "max": 6.0},
    }
    # Safe open-interval clip: validator requires STRICTLY (0, 1)
    # Use 0.05 / 0.95 as the user requires, but we keep them away from 0 and 1
    _SCORE_CLIP_MIN = 0.05
    _SCORE_CLIP_MAX = 0.95
    ...
    def __init__(self, max_steps: int = MAX_STEPS) -> None:
        ...
        # OpenEnv validator requires env.tasks — dict of task_id → task config with grader
        self.tasks: Dict[str, Any] = {
            tid: {
                "grader":      grade_task,
                "grader_fn":   grade_task,
                "max_steps":   max_steps,
                "description": f"SENTINEL-PR task: {tid}",
            }
            for tid in _TASK_REGISTRY
        }
    ...
    def step(self, action_dict: Any) -> StepResult:  # noqa: C901
        ...
        info: Dict[str, Any] = {
            ...
            "victory":           victory,
            "fixed_issues":      len(self._fixed_keys),
            ...
            # score: always present, always strictly in (0, 1)
            "score": max(0.05, min(0.95, self._compute_score(self._episode_reward))),
        }
    ...
    def _compute_score(self, reward: float) -> float:
        ...
        if self._victory:
            normalised = max(normalised, 0.85)
...
# ---------------------------------------------------------------------------
# Task 4 — Insecure Deserialization
# ---------------------------------------------------------------------------
_TASK4_SRC = textwrap.dedent("""\
    import pickle, base64, os
    from flask import Flask, request, jsonify

    app = Flask(__name__)

    def load_user_session(session_b64: str) -> dict:
        \"\"\"Deserialise a user session from a base64-encoded pickle blob.\"\"\"
        raw = base64.b64decode(session_b64)
        return pickle.loads(raw)            # CWE-502: untrusted deserialisation
    ...
    def login():
        ...
        if session_cookie:
            user = load_user_session(session_cookie)
            return jsonify(user)
        return jsonify({"error": "no session"}), 401

    SECRET_TOKEN = "hardcoded-admin-token-123"   # CWE-798
""")
_TASK4_MFT = "flask==2.3.3\npyyaml==5.4.1\nrequests==2.28.2\n"

# Issues present: pickle.loads (CRITICAL/B301), hardcoded SECRET_TOKEN (HIGH/B105)
_TASK4_ISSUES = [
    {"issue_id": "B301", "severity": "CRITICAL", "line_no": 8,
     "description": "pickle.loads on untrusted data — CWE-502 RCE",
     "evidence": "pickle.loads(raw)"},
    {"issue_id": "B105", "severity": "HIGH", "line_no": 18,
     "description": "Hardcoded token — CWE-798",
     "evidence": "SECRET_TOKEN = \"hardcoded-admin-token-123\""},
]


def _grade_task4(episode_result: Dict[str, Any]) -> float:
    """
    Grader for task_insecure_deserialization.
    score = 0.2 + (fixed/total)*0.6 + 0.2*validation_pass
    Clamped to (0.06, 0.94).
    """
    total_issues   = 2
    fixed_issues   = int(episode_result.get("fixed_issues", 0))
    validation_pass = int(bool(episode_result.get("victory", False)))
    raw = 0.2 + (min(fixed_issues, total_issues) / total_issues) * 0.6 + 0.2 * validation_pass
    return round(min(0.94, max(0.06, raw)), 4)


# ---------------------------------------------------------------------------
# Task 5 — Command Injection
# ---------------------------------------------------------------------------
_TASK5_SRC = textwrap.dedent("""\
    ...
    def ping():
        ...
        # CWE-78: unsanitised shell command
    ...
    def lookup():
        ...
        out = subprocess.call(f"nslookup {domain}", shell=True)
        return jsonify({"output": out})

    API_SECRET = "super-secret-api-key-456"    # CWE-798
""")
_TASK5_MFT = "flask==2.3.3\nrequests==2.28.2\nurllib3==1.26.18\n"

# Issues present: os.system (CRITICAL/B605), subprocess.call (HIGH/B603), hardcoded key (HIGH/B105)
_TASK5_ISSUES = [
    {"issue_id": "B605", "severity": "CRITICAL", "line_no": 9,
     "description": "os.system with unsanitised input — CWE-78",
     "evidence": "os.system(f\"ping -c 1 {host}\")"},
    {"issue_id": "B603", "severity": "HIGH", "line_no": 15,
     "description": "subprocess.call with shell=True — CWE-78",
     "evidence": "subprocess.call(f\"nslookup {domain}\", shell=True)"},
    {"issue_id": "B105", "severity": "HIGH", "line_no": 19,
     "description": "Hardcoded API secret — CWE-798",
     "evidence": "API_SECRET = \"super-secret-api-key-456\""},
]


def _grade_task5(episode_result: Dict[str, Any]) -> float:
    """
    Grader for task_command_injection.
    score = 0.2 + (fixed/total)*0.6 + 0.2*validation_pass
    Clamped to (0.06, 0.94).
    """
    total_issues    = 3
...
# ---------------------------------------------------------------------------
_TASK_GRADERS: Dict[str, Any] = {}  # populated after SentinelPREnv is defined


def _grade_task_eval_auth_flaw(episode_result: Dict[str, Any]) -> float:
    """
    Grader for task_eval_auth_flaw.
    Issues: eval x2 (CRITICAL), hardcoded SECRET (HIGH), timing oracle (MEDIUM) = 4 total.
    score = 0.2 + (fixed/total)*0.6 + 0.2*validation_pass
    Clamped to (0.06, 0.94).
    """
    total_issues    = 4
...
# ── GRADER HELPERS ────────────────────────────────────────────────────────────
# Score bounds: STRICTLY (0.06, 0.94) — validator requires strictly (0, 1).
# 0.05 and 0.95 are the validator boundary → we use 0.06 and 0.94 to be safe.
_SCORE_LO = 0.05
_SCORE_HI = 0.95


def _clamp(x: float) -> float:
    """Clamp to strictly open interval — never returns 0.0 or 1.0."""
    return round(min(_SCORE_HI, max(_SCORE_LO, x)), 4)


def _grade(fixed: int, total: int, victory: bool) -> float:
    """
    Shared formula: score = 0.2 + (fixed/total)*0.6 + 0.2*victory
    Output: strictly in (_SCORE_LO, _SCORE_HI).
    """
    if total <= 0:
        total = 1
    ratio = min(fixed, total) / total
    raw   = 0.2 + ratio * 0.6 + 0.2 * int(victory)
    return _clamp(raw)


def _grade_task1(episode_result: Dict[str, Any]) -> float:
    """Grader: task_hardcoded_key (1 issue)."""
    return _grade(
        int(episode_result.get("fixed_issues", 0)),
        1,
        bool(episode_result.get("victory", False)),
    )


def _grade_task2(episode_result: Dict[str, Any]) -> float:
    """Grader: task_vulnerable_dep (3 issues)."""
    return _grade(
        int(episode_result.get("fixed_issues", 0)),
        3,
        bool(episode_result.get("victory", False)),
...
def grade_task(task_id: str, episode_result: Dict[str, Any]) -> float:
    """
    Public grader entry-point.
    Returns a score STRICTLY in (0.06, 0.94) — never 0.0, 0.05, 0.95, or 1.0.
    """
    fn = _TASK_GRADERS.get(task_id)
    if fn is None:
        reward  = float(episode_result.get("total_reward", 0.0))
        victory = bool(episode_result.get("victory", False))
        raw     = 0.2 + 0.2 * int(victory) + min(max(reward, -10.0), 10.0) * 0.02
        return _clamp(raw)
    return fn(episode_result)
...
_TASK_REGISTRY: Dict[str, Observation] = {
    ...
    "task_insecure_deserialization":Observation(source_code=_TASK4_SRC, dependency_manifest=_TASK4_MFT, security_policy=_DEFAULT_POLICY),
    "task_command_injection":       Observation(source_code=_TASK5_SRC, dependency_manifest=_TASK5_MFT, security_policy=_DEFAULT_POLICY),
}

# Wire graders for ALL 5 tasks
_TASK_GRADERS.update({
    "task_hardcoded_key":            _grade_task1,
    "task_vulnerable_dep":           _grade_task2,
    "task_eval_auth_flaw":           _grade_task_eval_auth_flaw,
    "task_insecure_deserialization": _grade_task4,
    "task_command_injection":        _grade_task5,
})
